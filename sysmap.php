...
if (isset($_REQUEST['favobj'])) {
	if (getRequest('favobj') === 'sysmap' && hasRequest('action')) {
		...
		elseif (getRequest('action') === 'expand') {
			error_log('Expand action triggered'); // Agregar depuración
			...
			foreach ($sources as $num => $source) {
				error_log('Processing source: ' . print_r($source, true)); // Agregar depuración
				if (is_array($source) && (array_key_exists('label', $source) || array_key_exists('text', $source))) {
					...
					if (array_key_exists('elementtype', $source) && array_key_exists('elements', $source)
						&& is_array($source['elements'])) {
						error_log('Element type set to selements'); // Agregar depuración
						$element_type = 'selements';
					} else {
						error_log('Element type set to ' . (array_key_exists('label', $source) ? 'links' : 'shapes')); // Agregar depuración
