...
// Obtener user groups del usuario
$usergroups = [];
...
// Verificar que se hayan obtenido user groups

/* echo 'User groups: <pre>';
print_r($usergroups);
echo '</pre>'; */

if (!empty($usergroups)){
	$usergroupList = implode(',',$usergroups);
	
	$sql = "SELECT DISTINCT h.groupid FROM hosts_groups h JOIN rights r ON h.groupid = r.id WHERE r.groupid IN ($usergroupList)";
	$result = DBselect($sql);
	$hostgroups = [];
	while ($row = DBfetch($result)) {
		$hostgroups[] = $row['groupid'];
	}
	 // Mostrar los hostgroups

	/*  echo 'Host groups: <pre>';
	 print_r($hostgroups);
	 echo '</pre>'; */

 } else {
	 echo 'No se encontraron hostgroups.';
 
}


// si existen  hostgroups, que siga con la consulta para obtener los hostid
if (!empty($hostgroups)) {
    // Crear una lista de valores para la consulta IN
    $hostgroupList = implode(',', array_map('intval', $hostgroups));
    
    // esta es la consulta SQL para obtener los hostid asociados a los hostgroups
    $sql = "SELECT DISTINCT hg.hostid
            FROM hosts_groups hg
            WHERE hg.groupid IN ($hostgroupList)";
    
    $result = DBselect($sql);
    
        // Almaceno los hostid en un array
        $hostids = [];
        while ($row = DBfetch($result)) {
            $hostids[] = $row['hostid'];
        }
 ...
 */
        ...
        echo 'No se encontraron hostids asociados a los hostgroups.';
    }

// obtengo el ID del host desde la URL si hay múltiples valores

if (isset($_GET['hostids'])) {
    $hostUrl = $_GET['hostids'];

    // verifico si $hostIds es un array
    if (is_array($hostUrl)) {
        foreach ($hostUrl as $hostUrl) {
            /* echo "ID del hostryy capturado desde la URL: $hostUrl <br>"; */
            
			foreach ($hostids as $hostid){
				/* echo " el hostid es ". $hostid; */
			}
			
			if(in_array($hostUrl, $hostids)){
				/* echo "true"; */
			} else {
        ...
        echo "ID del host capturado desde la URL: $hostUrl";
        
    }
} else {
    /* echo "No se encontró el parámetro 'hostids' en la URL."; */
}



if (class_exists('CWebUser') &&!is_null(CWebUser::$data)) {
    $roleId = CWebUser::$data['roleid'];
}
